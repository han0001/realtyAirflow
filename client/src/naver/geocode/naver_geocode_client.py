...
from client.src.naver.geocode.response.naver_geocode_response import AddressSeachResponse
...
class NaverGeocodeClient:
    ...
    def get_geocode_one(self, query):
        """
        주소 검색
        지번, 도로명를 질의어로 사용해서 주소 정보를 검색합니다. 검색 결과로 주소 목록과 세부 정보를 JSON 형태로 반환합니다.
        """
        try:
            ...
            response.raise_for_status()
            # if len(response.json()['addresses']) > 1:
            #     raise requests.HTTPError("1개 이상의 결과 반환")
        except requests.RequestException as e:
            raise requests.HTTPError(f"An error occurred while fetching data: {e}")

        return AddressSeachResponse(**response.json()['addresses'][0])